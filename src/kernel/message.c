/*
** $Id: message.c 13633 2010-11-20 06:16:29Z wanzheng $
**
...
{
  printf("send message start --> hWnd %ld iMsg %d wParam %ld lParam %ld \n",hWnd,iMsg,wParam,lParam);
  WNDPROC WndProc;

  MG_CHECK_RET (MG_IS_WINDOW(hWnd), -1);

#ifdef _MGRM_THREADS
  printf("this is thread mode \n");
  if (!BE_THIS_THREAD(hWnd))
    return SendSyncMessage (hWnd, iMsg, wParam, lParam);
  else 
    printf("In the same thread\n");
#endif
...
{
  printf("send Sync Message start\n");
